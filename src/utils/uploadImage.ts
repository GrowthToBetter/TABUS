...
export async function UploadImageCloudinary(file: Buffer | any) {
  try {
    ...
    const upload: UploadApiResponse | undefined = await new Promise((resolve, reject) => {
      cloudinary.uploader
        .upload_stream({ upload_preset: "tabusUserProfile", quality: "auto:best" }, (error, result) => {
...
export async function UploadImageCloudinaryFile(file: Buffer | any) {
  try {
    if (!file || (!file.data && !file.length)) {
      return { error: true, message: "No file provided or file is empty" };
    }
    const upload: UploadApiResponse | undefined = await new Promise((resolve, reject) => {
      cloudinary.uploader
        .upload_stream({ upload_preset: "tabusCoverFile", quality: "auto:best" }, (error, result) => {
          if (error) reject(error);
          return resolve(result);
        })
        .end(file?.data ? file.data : file);
